...
__export(index_exports, {
  ...
  createQuoteEngineComposable: () => createQuoteEngineComposable,
...
// src/vue/index.ts
var QuoteEngineKey = /* @__PURE__ */ Symbol("QuoteEngine");
function createQuoteEngineComposable(config) {
  ...
  const streamingContent = (0, import_vue.ref)("");
  async function startChat(customerData) {
  ...
  const messages = (0, import_vue.computed)(() => state.chatSession?.messages || []);
  const shouldGenerateQuote = (0, import_vue.computed)(
    () => messages.value.some((m) => m.metadata?.shouldGenerateQuote)
  );
  return {
    // Client
    client,
...
function useQuoteEngine() {
  const quoteEngine = (0, import_vue.inject)(QuoteEngineKey);
  if (!quoteEngine) {
    throw new Error(
...
function useQuoteChat() {
  ...
  return {
    ...
    isLoading: (0, import_vue.computed)(() => qe.state.isLoading),
    error: (0, import_vue.computed)(() => qe.state.error),
...
function useQuote() {
  const qe = useQuoteEngine();
  return {
    quote: (0, import_vue.computed)(() => qe.state.quote),
    isLoading: (0, import_vue.computed)(() => qe.state.isLoading),
    error: (0, import_vue.computed)(() => qe.state.error),
    generateQuote: qe.generateQuote,
    downloadPdf: qe.downloadPdf,
    sendEmail: qe.sendEmail,
    getQuote: (id) => qe.client.getQuote(id)
  };
}
function usePayment() {
  const qe = useQuoteEngine();
  const isProcessing = (0, import_vue.ref)(false);
  const paymentError = (0, import_vue.ref)(null);
  const clientSecret = (0, import_vue.ref)(null);
  const paymentIntentId = (0, import_vue.ref)(null);
  const escrowInfo = (0, import_vue.ref)(null);
  async function initPayment(type = "deposit", milestoneId, returnUrl) {
...
function useQuoteWidget() {
  const qe = useQuoteEngine();
  return {
    isOpen: (0, import_vue.computed)(() => qe.state.isOpen),
    isLoading: (0, import_vue.computed)(() => qe.state.isLoading),
    currentStep: (0, import_vue.computed)(() => qe.state.currentStep),
    error: (0, import_vue.computed)(() => qe.state.error),
    customerData: (0, import_vue.computed)(() => qe.state.customerData),
    open: qe.open,
    close: qe.close,
    reset: qe.reset,
...
function useQuoteEngineEvent(event, handler) {
  ...
  (0, import_vue.onMounted)(() => {
    const unsubscribe = qe.client.addEventListener(event, handler);
