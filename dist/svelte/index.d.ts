 ...
 * ```
 */
declare function createQuoteEngineStore(config: QuoteEngineConfig): QuoteEngineStore;
/**
 * Context key for Quote Engine store
 */
declare const QUOTE_ENGINE_CONTEXT_KEY = "quote-engine";

export { Analytics, type AnalyticsEvent, type AnalyticsEventType, type AnalyticsOptions, type ApiKeyEnvironment, type ApiKeyErrorCode, type ApiKeyPermissions, type ApiKeyScope, ApiKeyValidationError, type ApiResponse, type CallbackHandlers, type ChatCompletedPayload, type ChatMessage, type ChatMessageV2, type ChatResponse, type ChatSession, type ClassNameOverrides, type Complexity, type ContractInfo, type ContractSignedPayload, type ConversationData, type Customer, type CustomerData, DEFAULT_DARK_THEME, type DeepPartial, type DepositInfo, type Discount, ELEGANT_THEME_DARK, ELEGANT_THEME_LIGHT, type EscrowInfo, type EscrowStatus, type ExtractedRequirements, type Feature, GLASS_THEME_DARK, GLASS_THEME_LIGHT, type Language, MINIMAL_THEME_DARK, MINIMAL_THEME_LIGHT, type MessageMetadata, PLAN_LIMITS, PLAN_PRICING, type ParsedApiKey, type PaymentEventPayload, type PaymentInitResponse, type PaymentInitResult, type PaymentMethod, type PaymentMilestone, type PaymentStatus, type PriceBreakdown, type PriceCategory, type Pricing, type Project, type ProjectPhase, type ProjectType, QUOTE_ENGINE_CONTEXT_KEY, type Quote, QuoteEngineClient, type QuoteEngineConfig, type QuoteEngineError, type QuoteEngineErrorCode, type QuoteEngineEvent, type QuoteEngineStore, type QuoteEventPayload, type QuoteGenerationResponse, type QuoteStatus, type QuoteStep, type QuoteV2, type RAGInsight, SCOPE_PERMISSIONS, type SimilarProjectRef, type StorageAdapter, type StreamEvent, type StreamEventType, THEME_STYLE_PRESETS, type Technology, type TenantPlan, type TenantPublicConfig, type TextLabels, type ThemeConfig, ThemeManager, type ThemeManagerOptions, type ThemeMode, type ThemeStyle, type Timeline, type Unsubscribe, VERSION, type VatInfo, WEBHOOK_ID_HEADER, WEBHOOK_SIGNATURE_HEADER, WEBHOOK_TIMESTAMP_HEADER, WebhookClient, type WebhookClientOptions, type WebhookEndpoint, type WebhookEvent, type WebhookEventType, type WebhookPayload, type WebhookPayloadBase, type WidgetMode, type WidgetOptions, type WidgetPosition, type WidgetState, assertPermission, calculatePricingSummary, computeSignature, constructEvent, createAnalytics, createAnalyticsFromConfig, createQuoteEngine, createQuoteEngineStore, createStyledTheme, createThemeManager, createWebhookClient, debounce, deepMerge, formatCurrency, formatDate, formatQuoteNumber, generateThemeCss, getApiKeyPermissions, getCountryFromLocale, getDaysUntilExpiry, getLocaleFromLanguage, getScopeDescription, getThemePreset, hasPermission, isLiveKey, isQuoteExpired, isTestKey, isValidApiKeyFormat, isValidEmail, isValidPhone, listEnabledPermissions, maskApiKey, parseApiKey, storage, t, throttle, translations, validateApiKey, verifySignature };
