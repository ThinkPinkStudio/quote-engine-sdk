...
__export(index_exports, {
  ...
  QUOTE_ENGINE_CONTEXT_KEY: () => QUOTE_ENGINE_CONTEXT_KEY,
  ...
  createQuoteEngineStore: () => createQuoteEngineStore,
...
// src/svelte/index.ts
function createQuoteEngineStore(config) {
  ...
  const streamingContent = (0, import_store.writable)("");
  const messages = (0, import_store.derived)(state, ($state) => $state.chatSession?.messages || []);
  const shouldGenerateQuote = (0, import_store.derived)(
    messages,
    ($messages) => $messages.some((m) => m.metadata?.shouldGenerateQuote)
  );
  async function startChat(customerData) {
    state.update((s) => ({ ...s, isLoading: true, error: null }));
    try {
      const currentState = (0, import_store.get)(state);
...
var QUOTE_ENGINE_CONTEXT_KEY = "quote-engine";
