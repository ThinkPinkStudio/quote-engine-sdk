...
var QuoteEngineMain = class {
  ...
  registerIPC() {
    let ipcMain;
    try {
      ipcMain = __require("electron").ipcMain;
    } catch {
      throw new Error(
    ...
    ipcMain.handle("qe:start-chat", async (_event, customerData) => {
      return this.client.startChat(customerData);
    });
    ipcMain.handle("qe:send-message", async (_event, content) => {
      const chunks = [];
    ...
    ipcMain.handle("qe:generate-quote", async (_event, customerData) => {
      return this.client.generateQuote(customerData);
    });
    ipcMain.handle("qe:get-quote", async (_event, quoteId) => {
      return this.client.getQuote(quoteId);
    });
    ipcMain.handle("qe:download-pdf", async (_event, quoteId) => {
      const blob = await this.client.downloadPdf(quoteId);
      const buffer = await blob.arrayBuffer();
      return Buffer.from(buffer);
    });
    ipcMain.handle("qe:send-email", async (_event, quoteId, email) => {
      return this.client.sendQuoteEmail(quoteId, email);
    });
    ipcMain.handle(
      "qe:init-payment",
      async (_event, quoteId, type, milestoneId, returnUrl) => {
        return this.client.initPayment(quoteId, type, milestoneId, returnUrl);
      }
    );
    ipcMain.handle(
      "qe:confirm-payment",
      async (_event, quoteId, paymentIntentId) => {
  ...
  createWindow(options = {}) {
    let BrowserWindow;
    try {
      BrowserWindow = __require("electron").BrowserWindow;
    } catch {
      throw new Error(
   ...
   */
  destroyWindow() {
    if (this.qeWindow && this.qeWindow.isDestroyed && !this.qeWindow.isDestroyed()) {
      this.qeWindow.destroy();
    }
    this.qeWindow = null;
  }
};
...
function exposeQuoteEngineAPI(apiName = "quoteEngine") {
  let contextBridge;
  let ipcRenderer;
  try {
  ...
  const api = {
    ...
    startChat: (customerData) => ipcRenderer.invoke("qe:start-chat", customerData),
    sendMessage: (content) => ipcRenderer.invoke("qe:send-message", content),
    generateQuote: (customerData) => ipcRenderer.invoke("qe:generate-quote", customerData),
    getQuote: (quoteId) => ipcRenderer.invoke("qe:get-quote", quoteId),
    downloadPdf: (quoteId) => ipcRenderer.invoke("qe:download-pdf", quoteId),
    sendEmail: (quoteId, email) => ipcRenderer.invoke("qe:send-email", quoteId, email),
    initPayment: (quoteId, type, milestoneId, returnUrl) => ipcRenderer.invoke("qe:init-payment", quoteId, type, milestoneId, returnUrl),
    confirmPayment: (quoteId, paymentIntentId) => ipcRenderer.invoke("qe:confirm-payment", quoteId, paymentIntentId),
